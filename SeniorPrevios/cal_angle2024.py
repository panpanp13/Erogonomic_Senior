...
profile=""
import pandas as pd
tablea = pd.read_csv(r'C:\Users\thinn\Desktop\CU\Senior project\Code\SeniorPrevios\Rula_score\TableA.csv')
tableb = pd.read_csv(r'C:\Users\thinn\Desktop\CU\Senior project\Code\SeniorPrevios\Rula_score\TableB.csv')
tablec = pd.read_csv(r'C:\Users\thinn\Desktop\CU\Senior project\Code\SeniorPrevios\Rula_score\TableC.csv')
#EDIT 28/12/2024#
...
def angle_calc(kpn, pose_list, reference_vector = None , head_theta0= None, back_theta0= None, vector0 = None):
    ...
    try :
        ...
        print(f"Rshoulder coordinate : {R_Shoulder}")
        ...
        if reference_vector is not None:
            ...
            sagittal = to_unitvec(sagittal)  # Normalize
            coronal = to_unitvec(np.cross(sagittal, reference_vector[3]))
            print("Sagittal vector:", sagittal)
            print("Coronal vector:", coronal)
            body_proj_upperarm = project_onplane(sagittal, -Back)
            RupperArm_proj = project_onplane(sagittal, RupperArm)
            body_proj_upperarm = to_unitvec(body_proj_upperarm)
            RupperArm_proj = to_unitvec(RupperArm_proj)
            print("RupperArm_proj (normalized):", RupperArm_proj)
            print("body_proj_upperarm (normalized):", body_proj_upperarm)
            Rshoulder = angle(RupperArm_proj, body_proj_upperarm)
            print(f"Rshoulder angle (before adjustments): {Rshoulder}")
            if np.linalg.norm(RupperArm_proj) == 0 or np.linalg.norm(body_proj_upperarm) == 0:
                print("Error: One of the projections is zero-length.")
                Rshoulder = 0
            else:
                dot_product = np.dot(RupperArm_proj, body_proj_upperarm)
                if dot_product < 0:
                    Rshoulder = 180 - Rshoulder
                print(f"Rshoulder angle (after adjustments): {Rshoulder}")
            print(f"corebody : {corebody}")
            corebody = to_unitvec(corebody)
            print(f"Unit corebody: {corebody}")
            Rshoulder2 = angle(RupperArm,corebody)
            print(f"Rshoulder with corebody : {Rshoulder2}")
        ...
        rula = rula_score(angle_dict,pose,profile,condition)
